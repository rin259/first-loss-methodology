...
\texttt{rin@rin.red}}
\date{Preprint candidate v1.1.0, September 2026}
...
This paper contributes (1) an evidence-lineage model with explicit loss and invariant boundaries; (2) rules for exact, bounded, nonlocal, and reacquired cases; (3) a confirmation protocol based on a frozen baseline and single-variable intervention; and (4) two reproducible evaluations, including three executable reference runtimes. The artifact is archived separately \cite{chen2026firstlossartifact}.
...
\section{Production incident replay}
...
\textbf{External validity.} Three executable runtimes are more informative than seven renamed instances, but they are small and deterministic. The study does not cover production concurrency, organizational handoffs, or unknown ground truth. One production replay shows plausibility, not prevalence.
...
The method also changes the order of work. Engineers first ask whether the evidence reached a stage, then whether it still meant the same thing, and only then whether that stage behaved correctly. This ordering does not make causal reasoning automatic, but it stops a common mistake: investigating a component as the cause of losing input it never received.
